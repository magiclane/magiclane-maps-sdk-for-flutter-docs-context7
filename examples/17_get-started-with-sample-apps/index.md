...
Example applications for the Maps SDK for Flutter are available on [Github](https://github.com/magiclane/magiclane-maps-sdk-examples-for-flutter). You can clone the repository or download project ZIP file by clicking the blue `Code` button and selecting `Download ZIP`.
...
And replace the `String.fromEnvironment('GEM_TOKEN')` with your actual API key:
```dart
const projectApiToken = '<token>';
```
