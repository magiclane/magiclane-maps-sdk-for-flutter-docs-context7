...
The `getLandmarkStoreById` method returns a valid `LandmarkStore` object if a landmark store exists with the given name and null if the given name does not correspond to any `LandmarkStore`.
