...
Calling `asyncGetStoreContentList` inside `onMapCreated` callback doesn't work.
...
Additionally, users can provide an optional `onProgressCallback` to receive real-time progress updates. This callback is triggered with the current download progress, represented as an integer between 0 and 100.
