...
//highlight-start
TaskHandler? handler = NavigationService.startNavigation(route, null,
...
<table>
  ...
  <tr>
    ...
    <td>Provides a text string for a maneuver that can be passed to a text-to-speech engine for audio guidance.</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td>Triggered when a better alternative route is detected, including the new route and details such as travel time, delays caused by traffic and time gains.</td>
  </tr>
  <tr>
...
The callbacks from the table provided above can be used for both simulation and navigation. There is still one exception, `onSkipNextIntermediateDestinationDetected()` doesn't make sense on a simulation.
